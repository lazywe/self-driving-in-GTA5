...
def batch_generator(image_paths, label_paths, batch_size, is_training):
    ...
    while True:
        ...
        for i in range(batch_size):
            ...
            if is_training:
                #screen = random_augment(np.load(image_paths[random_index]))  # I'll cancel the enhancement, but you can try.
...
plot_model(model,to_file='model_pic.png',show_shapes=True)   #visualize the network

### matplotlib for visualization ###
plt.plot(hst.history['loss'])
plt.plot(hst.history['val_loss'])
plt.plot(hst.history['acc'],label='acc')
plt.plot(hst.history['val_acc'],label='val_acc')
plt.title('Loss-Acc')
plt.xlabel('epoch')
plt.show()
